...
export default function CustomTraceWidget (
      ...
      for (let i = 0; i < items.length; i += batchSize) {
        ...
        try {
          ...
          const createDisplayFieldComplement = (attributes) => {
            if(attributes.ASSETGROUP === 12 && attributes.designtype === 1 && !(attributes.keyaccount > 1)){
              return "(Nierdialyse)"
            }
            else if(attributes.ASSETGROUP === 12 && attributes.designtype === 1 && attributes.keyaccount > 1) {
              return "(Nierdialyse en Key Account)"
            }
            else if(!(attributes.ASSETGROUP === 12 && attributes.designtype === 1) && attributes.keyaccount > 1){
              return "(Key Account)"
            }
  ...
  const transformObjectIntoArrayWithReadableKeys = (data) => {
    ...
    const processedData = sortedData.map(group => {
      ...
      // Include kidneyDialysis, kidneyDialysis+keyAccounts service connections
      const kidneyDialysis = group.items
        .filter(item => (item.ASSETGROUP === 12 && item.designtype === 1))
        .sort(sortByDisplayField);

      // Exclude kidneyDialysis+keyAccounts service connections
      const keyAccounts = group.items
        .filter(item => !(item.ASSETGROUP === 12 && item.designtype === 1) && item.keyaccount > 1)
        .sort(sortByDisplayField);
        ...
        {activeTab === 'results' && <div>
          ...
          <div>
            {groupedTraceResults_array.map((group, index) => (
              <div key={index}>
                <h4>{`${group.assetGroupCode} (${group.items[0].length+group.items[1].length+group.items[2].length})`}</h4>
                <ul style={{ listStyle: 'none', padding: 0}}>
